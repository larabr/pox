...
_unfix_map = {k:_unfix_null for k in list(of.ofp_match_data.keys())}
...
def list_switches (ofnexus = None):
  ...
  for dpid,con in ofnexus._connections.items():
    ports = []
    for p in list(con.ports.values()):
      pdict = {
        'port_no':p.port_no,
        'hw_addr':str(p.hw_addr),
        'name':p.name}
      for bit,name in list(of.ofp_port_config_map.items()):
        if p.config & bit:
          pdict[name.split('OFPPC_', 1)[-1].lower()] = True
      for bit,name in list(of.ofp_port_state_map.items()):
        if p.state & bit:
